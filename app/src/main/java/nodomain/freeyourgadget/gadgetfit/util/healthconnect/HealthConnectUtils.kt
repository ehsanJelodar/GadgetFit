...
package nodomain.freeyourgadget.gadgetfit.util.healthconnect
...
import nodomain.freeyourgadget.gadgetfit.entities.HealthConnectSyncState
...
import nodomain.freeyourgadget.gadgetfit.util.healthconnect.HealthConnectPermissionManager.PREF_KEY_LAST_GRANTED_HC_PERMISSIONS
import nodomain.freeyourgadget.gadgetfit.util.healthconnect.syncers.*
...
class HealthConnectUtils {
    ...
    private suspend fun healthConnectDataSyncImp(
        ...
        for (targetAddress in selectedDevices) {
            ...
            dataTypeLoop@ for (dataType in HealthConnectPermissionManager.HealthConnectDataType.entries) {
                ...
                while (currentSliceStartTs.isBefore(currentDataTypeEndTsFromDb)) {
                    ...
                    LOG.info("$HC_SYNC_TAG Querying gadgetfit DB for {}({}) from {} to {}", gbDevice.aliasOrName, dataType.name, queryStartTs, currentSliceEndTs)
        ...
        if (totalDataTypesProcessed == 0 && totalDataTypesSkipped > 0) {
            LOG.info("$HC_SYNC_TAG Health Connect sync completed. No data was available to sync. {} data types were skipped (no data in gadgetfit database or no permission).", totalDataTypesSkipped)
        } else {
